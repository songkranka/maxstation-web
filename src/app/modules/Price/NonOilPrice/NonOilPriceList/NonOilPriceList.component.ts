...
import { ModelMasEmployee, ModelPriNonoilHd } from 'src/app/model/ModelScaffold';
import { DefaultService } from 'src/app/service/default.service';
import { SharedService } from 'src/app/shared/shared.service';
import Swal from 'sweetalert2';
import { ModelNonOilPriceParam } from '../ModelNonOilPrice';
import { NonOilPriceService } from '../NonOilPriceService.service';

@Component({
  selector: 'app-NonOilPriceList',
  templateUrl: './NonOilPriceList.component.html',
  styleUrls: ['./NonOilPriceList.component.scss']
})
export class NonOilPriceListComponent implements OnInit {

  constructor(
    public SvDefault: DefaultService,
    public _svShared: SharedService,
    public _svStandardPrice: NonOilPriceService,
  ...
  dataSource = new MatTableDataSource<ModelPriNonoilHd>();
  pageEvent: PageEvent;
  ...
  private async getArrayHeader(pNumPage?: number) {
    ...
    let param = new ModelNonOilPriceParam();
    param.Page = numPage;
    ...
    let apiResult = await this._svStandardPrice.GetArrayHeader(param);
    if (apiResult != null) {
  ...
  public GetEmployeeName(pHeader: ModelPriNonoilHd) {
    let result: string;
  ...
  private getEmployeeName(pHeader: ModelPriNonoilHd) {
    if (pHeader == null) {
      return "";
    }
  ...
  private async onClickAddDocument() {
    ...
    window.location.href = "./NonOilPrice/New";
  }
