...
@Component({
    selector: 'app-AdjustRequestList',
    templateUrl: './AdjustRequestList.component.html',
    styleUrls: ['./AdjustRequestList.component.scss']
})
...
export class AdjustRequestListComponent implements OnInit {
    ...
    pageEvent: PageEvent;
    displayedColumns: string[] = ['no', 'docno', 'docdate', 'reasondesc', 'docstatus'];
    no = 0;
    length = 0;
    pageSize = 10;
    ...
    async findByValue(value: string) {
        let response = await this.AdjustRequestService.findAll(this.sharedService.brnCode, this.sharedService.compCode, this.sharedService.locCode, this.filterValue, this.dateRange.value.start, this.dateRange.value.end, 1, this.pageSize)
            .toPromise();
    ...
    navigateToProfile(id) {
        this.router.navigate(['/AdjustRequest/' + id], { relativeTo: this.activatedRoute });
    }
    ...
    async onPaginateChange(event: PageEvent) {
        ...
        if (this.filterValue == null) {
            ...
            let response = await this.AdjustRequestService.findAll(this.sharedService.brnCode, this.sharedService.compCode, this.sharedService.locCode, this.filterValue, this.dateRange.value.start, this.dateRange.value.end, page, this.pageSize)
                .toPromise();
            response.items = this.convertDocStatus(response.items);
            this.dataSource.data = this.toTableData(response.items);
            this.length = response.totalItems;
    ...
    private toTableData(obj: AdjustRequest[]): TableData[] {
        return obj.map(item => {
            return {
    ...
    convertDocStatus(obj: AdjustRequest[]) {
        obj.forEach(element => {
