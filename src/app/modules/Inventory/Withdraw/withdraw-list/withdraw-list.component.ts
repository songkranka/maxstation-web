...
import { WithdrawData, Withdraw } from 'src/app/model/inventory/withdraw.interface';
...
export class WithdrawListComponent implements OnInit, AfterViewInit {
  ...
  pageEvent: PageEvent;
  displayedColumns: string[] = ['no', 'docno', 'docdate', 'useby', 'docstatus'];
  no = 0;
  length = 0;
  pageSize = 10;
  ...
  async findByValue(value: string) {
    let response = await this.withdrawService.findAll(this.sharedService.brnCode, this.sharedService.compCode, this.sharedService.locCode, value, this.dateRange.value.start, this.dateRange.value.end, 1, this.pageSize)
      .toPromise();
  ...
  navigateToProfile(id) {
    this.router.navigate(['/Withdraw/' + id], { relativeTo: this.activatedRoute });
  }
  ...
  async onPaginateChange(event: PageEvent) {
    ...
    if (this.filterValue == null) {
      ...
      let response = await this.withdrawService.findAll(this.sharedService.brnCode, this.sharedService.compCode, this.sharedService.locCode, this.filterValue, this.dateRange.value.start, this.dateRange.value.end, page, size)
        .toPromise();
      this.dataSource.data = this.toTableData(response.items);
      this.length = response.totalItems;
    } else {
  ...
  private toTableData(books: Withdraw[]): TableData[] {
    return books.map(b => {
      return {
        docdate: b.docDate,
        docno: b.docNo,
